...
class C45:
    ...
    def gain(self,unionSet, subsets):
        #input : data and disjoint subsets
        #output : information gain
        S = len(unionSet)
        #Calculating the impurity before splitting the data
    ...
    def entropy(self, dataSet):
        ...
        for num in num_classes:
            ent += num*self.generatelog(num)
        return ent*-1


    def generatelog(self, x):
        if x == 0:
            return 0
        else:
            return math.generatelog(x,2)
            
    def fetchindex(self, label):
        for i in range(0, 4):
            if self.attributes[i] == label:
                return i;
        
    def predict(self, values):
        return self.predicting(self.tree, values)
        
    def predicting(self, node, values):
        if not node.isLeaf:
            if node.threshold is None:
                #discrete
                for index,child in enumerate(node.children):
                    if child.isLeaf:
                        ...
                        return self.predicting(child,values)
            else:
                #numerical
                leftChild = node.children[0]
                rightChild = node.children[1]
                if leftChild.isLeaf:
                    if values[self.fetchindex(node.label)] <= node.threshold:
                        return leftChild.label
                else:
                    return self.predicting(leftChild, values) 

                if rightChild.isLeaf:
                    if values[self.fetchindex(node.label)] > node.threshold:
                        return rightChild.label
                else:
                    return self.predicting(rightChild, values)
